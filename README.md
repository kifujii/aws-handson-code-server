...
1つのAWSアカウント・リージョンで複数の環境を払い出す場合、以下のサービスクォータを考慮してください。

### VPC 数の制限

AWS のデフォルト VPC 上限は **リージョンあたり 5 つ** です。このコードは環境ごとに 1 つの VPC を作成します。デフォルト VPC (1 つ) がすでに存在するため、**同一アカウント・リージョンで最大 4 環境** が目安です。

> 上限引き上げを申請すれば 5 環境以上も可能ですが、反映に数日かかる場合があります。

### Bedrock クォータ

参加者全員が同時に Claude を呼び出すため、複数環境を同一アカウントで運用すると Bedrock のレート制限に達しやすくなります。

| 環境数 | 参加者数 (例) | 注意点 |
|--------|:----------:|--------|
| 1 環境 | ~20 人 | クォータに余裕があれば問題なし |
| 2 環境 | ~40 人 | InvokeModel の引き上げを推奨 |
| 3 環境以上 | ~60 人以上 | **別アカウント分割を強く推奨** |

> Bedrock クォータの確認・引き上げ: AWS コンソール → **Service Quotas** → **Amazon Bedrock** → `InvokeModel` / `InvokeModelWithResponseStream`

### EC2 vCPU 制限

デフォルトのインスタンスタイプ `m6i.8xlarge` (32 vCPU) を使用します。Running On-Demand Standard (A, C, D, H, I, M, R, T, Z) Instances のクォータが **環境数 × 32 vCPU** 以上あることを確認してください。
...
| 参加者数 | 推奨アカウント数 | 備考 |
|---------|:--------------:|------|
| ~40 人 | 1 アカウント (2 環境) | Bedrock クォータ引き上げを実施 |
| ~80 人 | 2 アカウント (各 2 環境) | アカウントを分けることでクォータ上限を回避 |
| ~160 人 | 4 アカウント (各 2〜4 環境) | Organizations での管理を推奨 |
